...
import edu.uoc.elearn.lti.provider.service.AgsRequestResolver;
...
public class AgsController {
	...
	private final AgsRequestResolver agsRequestResolver;

	@RequestMapping(method = RequestMethod.GET)
	public ModelAndView list(ToolProvider toolProvider) {
		AgsBean agsBean = agsRequestResolver.list(toolProvider, TAG);
	...
	public String createLineItem(String label, Double maxScore, ToolProvider toolProvider) {
		agsRequestResolver.createLineItem(label, maxScore, toolProvider, TAG);
		return "redirect:/ags";
	}
	...
	public ModelAndView index(@RequestParam("id") String id, ToolProvider toolProvider) {
		final LineItemBean lineItemBean = agsRequestResolver.get(id, toolProvider);
	...
	public String score(@RequestParam("id") String id, String userId, Double score, String comment, ToolProvider toolProvider) {
		final boolean scored = agsRequestResolver.score(id, userId, score, comment, toolProvider);
		if (!scored) {
